...
url = f'https://aclanthology.org/events/naacl-2022'
output = 'naacl_22.json'
...
def execute():
    ...
    target = soup.find('div', attrs={'id': '2022naacl-main'})
    
    first = True
    for link in target.find_all('p', attrs={'class': 'd-sm-flex align-items-stretch'}):
        # Skip the first entry, as it is the full proceedings
        if first:
            first = False
            continue
